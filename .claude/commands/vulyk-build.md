...
--stamp $stamp` (it repeats `## Verification` the entry's own `repeat` times, C2); exit 4 is a
miss whatever its `error` says (a red verification, `returned` not `DONE`, or `returned: missing`,
or - the existing classification, unchanged - a non-empty return that is not a report at all,
`worker returned no report`) - reading `STATUS:` by eye is allowed only for the one-line log,
never for this decision. **First** miss on a story - a `close-story` exit 4, or one of the two
worker-report misses, in any order - dispatch one fresh worker with the failure stated as a
condition to satisfy (a `worker threw`/`worker returned empty`/`worker returned no report` miss:
state that string plainly), then `close-story` again once it answers exit 0; the story stays
`todo`/`in-progress` for the next `status` either way, and the rest of the wave is untouched.
**Second** miss on the *same* story, any mix of the kinds - edit its `status:` to `blocked`,
append a `## Findings` line naming the miss (a `close-story` exit 4's own `error`, or the C3
string - `worker threw: ...`, `worker returned empty - ...`, or `worker returned no report`),
dispatch `lead-architect` with the story file and both failures, journal the stop (`03-building`,
"story blocked" / that same string), release the semaphore, and **end the loop entirely** - never
`open-round` on a wave carrying a blocked story. |
