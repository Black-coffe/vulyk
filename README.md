...
- **The cycle closes with a person, and every stage leaves a file.** Since v0.11.0 the pipeline is the six-stage loop in [docs/cycle.md](docs/cycle.md) - spec, plan, code, tests, **human**, ship - and a stage is closed by its confirmation on disk: `brief.md`, `**Approved:**`, `**Branch:**`, `acceptance.jsonl`, `**Checked:**`, `**Shipped:**`. The build lives on its own branch; the blind gate walks the *client's path*, not only the suite; and after `PASS` nothing moves until the owner has looked with their own eyes and `scripts/human-check.sh` has written down what they saw, pinned to the commit. `/vulyk-ship` refuses without it - the same way `/vulyk-build` refuses without approval - and what it publishes is pressed by a human, never by an agent.
...
| `/vulyk-review [scope]` | Adversarial `lead-review` pass + blind acceptance; blocks on critical findings; `PASS` ends in the owner's own look, recorded |
| `/vulyk-ship [slug]` | Stage 06: refuses without the owner's check, then version + CHANGELOG, merge, the human publishes, recorded - and the next circle's draft handed over |
