...
| ⚖️ Council | `council-haiku` / `council-sonnet` / `council-opus` | Haiku / Sonnet / Opus | Blind verdict on the brief's `## Asks`, from a reduced worktree — black-box client path / suite + each ask / intent & edge cases | Only what a client or the suite would see — never the stories |
| ⚖️ Council | `cycle-clerk` | `haiku` | The Workflow driver's only shell access — runs one `cycle.sh`/`journal.sh` verb per dispatch, no logic of its own | Never |
...
- **The cycle closes with a council, and one human stop at the start.** The pipeline is the six-stage loop in [docs/cycle.md](docs/cycle.md) - spec, plan, code, tests+human, ship - and a stage is closed by its confirmation on disk: `brief.md`, `**Briefed:**` (or `**Approved:**` for an owner who opts back into plan approval), `**Branch:**`, `council.jsonl`, `**Shipped:**`. The one human stop autonomous mode keeps is the grill inside `/vulyk-plan`: a single round, recommended options first, closing into the brief's `## Asks`. From there the build lives on its own branch, and a three-seat blind council (black-box client path, suite-then-each-ask, intent and edge cases) judges only those asks from a worktree that cannot see the stories - its verdict computed by a model-free script, not read off a person's look. `/vulyk-ship` refuses without a current, GREEN verdict the same way `/vulyk-build` refuses without a briefed plan, and what it publishes is pressed by a human, never by an agent.
...
| `/vulyk-plan <goal>` | Queen mode: scouts recon, a one-round grill (recommended options first, silence safe) closes into the brief's `## Asks`, plan + story files written to `docs/specs/`, tier assigned, then straight into the build |
| `/vulyk-build [story]` | Execute the plan wave by wave: parallel workers on disjoint files, one commit per story |
| `/vulyk-review [slug]` | One council round on demand: `lead-review` ∥ three blind seats (`council-haiku`/`council-sonnet`/`council-opus`), judged by `cycle.sh` from evidenced reports - `RED` routes to `/vulyk-build` as fix stories |
...
**Recorded by `scripts/acceptance-log.sh`** — the pre-council path, still live for any spec recorded before v0.12.0's council replaced it. The verdict of `drone-acceptance` — which read the human's brief and the repository, never the plan or the stories — next to what the stories claimed about themselves, appended to `memory/stats/acceptance.jsonl`. The number worth having is the disagreement: every story `done` while the blind gate did not accept. It is the only series in this framework capable of contradicting the framework's own account of itself. It starts empty on a fresh install; the first real entries are in, and what they showed is below. Each entry names the commit and a fingerprint of the pack it judged, because a verdict is about the stories that existed when the drone ran — let review carve out repair stories afterwards and the old verdict is about work that never shipped. `acceptance-log.sh --check <spec-dir>` answers whether the newest verdict still describes the spec: `CURRENT`, `STALE`, or `NO VERDICT RECORDED`. Read `drift` for what it is — evidence about delivery, never about proof. The two are independent: on the first real run the blind gate accepted a pack `lead-review` then blocked twice, and both were right.
...
- [x] **v0.9.0 — memory hardening.** [The gates reference](docs/pipeline.md): what each gate structurally cannot see, and when it stops being true. A `## Profile` block every caste starts from, whose last row states which configurations actually exist — the thing a reviewer needs before it can demand a guarantee. ADR harvest from `## Plan deltas` on the review PASS path, proposed-only and forbidden from inventing options nobody weighed. And `drone-docs` moved off the worker's account onto the diff, with a verify-before-write checklist.
- [x] **v0.9.1 — optional instruments.** `scripts/state.sh` derives `.claude/state.json` from story frontmatter and nothing else. Gitignored on purpose: a derived artifact committed beside its source is a second account of the same fact, which is failure mode four built by hand. Statuses outside the convention are counted as `unrecognised`, never as `todo` — the first real run reported a repository's older specs as 0-of-28 done, and a view that miscounts quietly is worse than no view.
...
- [x] **v0.11.0 — the cycle closes with a person.** The pipeline had four of the six stages a delivery loop needs and stopped at *merge*: no branch per spec, no named walk down the client's path, no stage where the owner looks, no publish, no next circle. Now [the cycle](docs/cycle.md) is named, each stage has a confirmation artifact on disk, and the two that were missing are the two an agent cannot take - the owner's own look (`scripts/human-check.sh`, pinned to the commit; `/vulyk-ship` refuses without it) and the publish (`/vulyk-ship` prints it, a human presses it, `ship-check.sh --record` writes it down). `ship-check.sh` reads all six confirmations at once and says which stage is open, deterministically and for free.
