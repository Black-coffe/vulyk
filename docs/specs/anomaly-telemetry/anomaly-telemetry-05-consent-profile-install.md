...
## Goal
The constitution's Profile table carries a `Telemetry` row, `off` by default, that `telemetry.sh consent` reads. `install.sh`, on a fresh install and on `--upgrade`, prints a short explanation of telemetry in the terminal and asks one yes/no question; the answer sets the row. When nobody can answer (no TTY, `--check`, piped, `vulyk-update.sh` non-interactive) there is no prompt and the row is, or becomes, `off`. `/vulyk-bootstrap` shows the current value instead of asking again. A generalised `wire_hook` wires `anomaly-scan.sh` on Stop and SessionEnd into existing hives. The installer never ships `memory/stats/anomalies.jsonl`. CI install-smoke cases and the local suite cover every branch.
...
- Do not block or loop on the prompt: one question, one read with a default of `off` on empty input or EOF. Never prompt without a TTY on stdin, never under `--check`.
...
## Map slice
recon/hooks-and-stats.md §5 (OWNED, `shippable()`, `ensure_gitignore()`, the `skills.json` seed precedent) and §1 (the `settings.json` hook groups); recon/weekly-and-publish.md Entry points (`vulyk-update.sh` hands off to the fetched release's `install.sh --upgrade`); ci.yml install-smoke case names at lines 313-386 (D4.7-D4.11); plan.md `## Contracts` (Consent row, `consent` verb, installer prompt, `wire_hook`) and A6, A10, A11, A13, resolved recon 5; memory/map/agents-and-commands.md `/vulyk-bootstrap`.
...
- [ ] Fresh `install.sh` with a TTY on stdin prints the 4-6 line explanation (what is collected = the enum names, what never, where it goes, default off) and the question `Enable telemetry? [y/N]`; `y`/`yes` writes the row as `on`, anything else (including empty) writes `off`. Without a TTY, or under `--check`, or with `VULYK_TELEMETRY=on|off` / `--telemetry on|off`, no question is printed and the row is set per A10.
- [ ] `install.sh --upgrade` into a hive whose marked Profile block is filled and lacks `| Telemetry |`: with a TTY asks the same question and appends exactly one line inside the block after the last table row; without a TTY appends it as `off`. A block whose row is already `on` or `off` is byte-identical after upgrade and no question is asked, unless `--telemetry ask` / `VULYK_TELEMETRY=ask` (or an explicit `on|off`) is given, in which case only that row's value changes. `--check` reports the pending insertion and writes nothing.
...
- [ ] `scripts/vulyk-update.sh` keeps stdin attached when it has a TTY and forwards `--telemetry <v>` / `VULYK_TELEMETRY` untouched to the fetched release's `install.sh --upgrade`; no other behaviour of the wrapper changes. Resolved recon 5 (whether `install.sh` already reads stdin, how the wrapper execs it) is answered in `## Implementation notes`.
...
- [ ] `ci.yml` install-smoke (always non-TTY): the "never touches an already-filled Profile block" case (recon: line 313) is amended to allow only the appended `Telemetry` row; cases added: row appended as `off` when missing; byte-identical when present; `VULYK_TELEMETRY=on` sets `on` on a fresh install; `--check` never writes the row; `--upgrade` wires the two `anomaly-scan.sh` entries idempotently.
- [ ] `tests/telemetry.test.sh` gains, in the section story 01 marked: `bash -n install.sh`; the non-TTY upgrade and `wire_hook` cases above; a TTY case driven through `script`/a pseudo-terminal if available, else `VULYK_TELEMETRY=ask` with `y` on stdin, asserting the explanation text names `off` as the default and the row lands as `on`.
