...
- R14 (corrected 2026-09-13 by story 29): the earlier line above claimed a cross-check "against the platform's Workflow reference … dated 2026-09-12 per brief.md `## Evidence` line 3"; that line (brief.md `## Evidence`, "Platform (code.claude.com/docs, 2026-09-12)") names only the version gate, the Pro `/config` flag, the opt-in rule and the script path - no script API - so the claim rested on a source that does not contain it (LR M2, N-m8). Real fetch: `https://code.claude.com/docs/en/workflows`, 2026-09-13. `agent(prompt, options)` - "What the saved script looks like": "`agent()` spawns one subagent"; the page's own example passes only `schema` and `label`, never shows `agentType`/`model`/`effort` as keys (model choice is only narrated under "Cost": "A model the script names for a stage counts as the per-invocation model", pointing on to the Agent SDK reference for the full input list). `parallel([thunks])` - same section: "`parallel()` runs a set of agent tasks at the same time and waits for all of them"; no call-signature example and no wording anywhere on a throwing/rejecting thunk. `pipeline(items, …stages)` - same section's own code is single-stage, `pipeline(found.files, file => agent(...))`; the driver's two-stage `pipeline(seats, seat => …, (report, seat) => …)` form is not shown or confirmed by this page. `phase(title)` - "Edit a saved script" > Body: "call `phase()` to group the agents that follow under a title in the progress view" - a standalone call; the page never lists `phase` as an `agent()` option, but `dispatchSeat` (`vulyk-cycle.js:71,74-77`) passes `phase: 'Round'` inside `agent()`'s options object - mismatch, flagged below. `log()` - same bullet: "call `log()` to show a message above the phases" - matches the driver's standalone `log(...)` calls. `meta` - "meta block": "keep `export const meta` … a plain object literal with a `name` and a `description`" plus "phases: … give each entry exactly the title you pass to `phase()`", and "Timestamps and randomness": "Claude Code makes `Date.now()`, `Math.random()`, and a no-argument `new Date()` throw inside the script … Pass a timestamp in through `args` instead" - matches the driver's literal `meta` and its `args.stamp` use (R31). Runtime behaviours: `agent()` returning `null` - "An `agent()` call resolves to `null` if you stop it mid-run or it hits an unrecoverable API error … A blocked call resolves to `null`" - CONFIRMED (triggers named are stop/API-error/auto-mode-block, not literally "aborted, timed out"); a throwing `parallel` thunk resolving to `null` - NOT stated anywhere on this page; a throwing `pipeline` stage dropping its item - NOT stated either: the page says pipeline "keeps each `null` in the results array" (the caller's own `.filter(Boolean)` drops it) - that covers an already-null `agent()` result, not a thrown exception from a stage. What changed: no call shape is outright contradicted except the `phase` key inside `agent()`'s options, which this page never sanctions; the two throw-behaviours R28/R29/R32 rest on are simply not addressed by this reference, so they stay unverified by VULYK rather than confirmed - story 26 owns `vulyk-cycle.js` and both gaps are named in this story's CONCERNS line.
