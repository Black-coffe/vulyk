...
- `close-story` commits are scoped to the story's own declared `## Files` (+ the story file), never `-A`: a concurrent wave's other in-progress story must not ride along in one story's commit.
- `open-round`'s "clean tree" precondition ignores exactly lib.sh's `paperwork_only()` whitelist (plan.md/journal.md/council/\*/stats jsonls) applied to `git status --porcelain`, not just the current spec's own `council/` - record-seat has no `--commit` of its own, so an in-flight round's seat files are legitimately uncommitted mid-round.
- Surprise/bug found and fixed: a round's own opening (or STALE-folding) commit necessarily moves HEAD past the `head=` it just recorded in `ROUND` - so the idempotent "HEAD unchanged" resume check now also accepts `paperwork_only(ROOT, RHEAD, HEAD)` between the recorded head and current HEAD (mirrors how `cmd_status`'s own `green` derivation already tolerates a paperwork-only gap), or every round reads itself as stale on the very next call.
- `judge` gains only unconditional best-effort court removal (`git worktree remove --force` + `rm -rf` fallback + `git worktree prune`) after its existing row/line/journal writes, for both the `judge` and `escalate` CLI verbs (they share one function body) - a missing court is not an error.
- `open-round`'s orphan cleanup (`clean_court`) removes every worktree - registered or leftover - under `.vulyk/court/<slug>/` before building a fresh one, rather than trying to distinguish "this round's old court" from "an earlier crash's leftover"; simpler and covers both D5 cases the same way.
- `reopen`'s brief.md append creates a `## Answers` heading at EOF if none exists yet (e.g. a Tier-1 mini-brief that skipped grill), instead of requiring one - the acceptance criteria only requires the decision land somewhere under that heading.
- `tests/council.test.sh` setup now copies `scope-check.sh` too and writes a synthetic `.gitignore` (`.vulyk/`, `docs/specs/*/PAUSE`) mirroring the real repo's - without it, `git status --porcelain` doesn't hide the court worktree or PAUSE files inside the throwaway repo and the "clean tree" precondition is untestable.
- Two PAUSE-guard fixture calls fixed to match the real interface: `close-story` takes a story-file (not a spec-dir) and `reopen` needs a decision string - both now pass a real argument so the guard, not a usage error, is what's actually exercised.
- CONCERN for a future story (not touched here, out of Non-goals): `record-seat`'s own staleness check (`cmd_record_seat`, `[ "$RHEAD" = "$HEAD" ]`, story 03) and `cmd_status`'s `stale` field (`STALE_B`, story 01) compare a round's recorded head to current HEAD with a *plain* equality, not `paperwork_only`-aware like the fix above - so the very first `record-seat` call after a real `open-round --commit` will read as exit 5 STALE even though no code moved (only the round's own paperwork commit did). Every existing test reaches record-seat only via the `mk_open_round` fixture helper (which stamps `head=<current HEAD at fixture time>` directly, sidestepping this), so it was never exercised end-to-end; this story's own tests use the same `write_seat` fixture shortcut for the same reason, per Non-goals ("do not change ... record-seat validation").
