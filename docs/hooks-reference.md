...
| `top-model-brief.sh` | SessionStart | Prints one `[VULYK] top model: ...` line: the alias `scripts/top-model.sh` resolved (`fable` on Max and premium seats, `opus` on Pro, standard seats and API), the plan it was read from, the Tier 4 second-reviewer pairing, and whether the Queen's own session is pinned to it in `.claude/settings.local.json`. Reads only; the pin is `scripts/top-model.sh --apply`, a decision. Silent without the resolver. Disable with `VULYK_TOP_MODEL_BRIEF=0`. |
